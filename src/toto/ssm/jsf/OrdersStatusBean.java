...
@ManagedBean(name = "ordersstatusbean")
@ViewScoped
public class OrdersStatusBean implements Serializable {
	private static final long serialVersionUID = 1L;
	private List<OrdersStatus> slave;
	private OrdersStatus selectedRow;
	...
	private void init() {
		...
		slave = session.querryAllOrdersStatus();
	}
	...
	public void btnSaveClick() {
		session.updateOrderStatus(selectedRow);
		init();
	}
	
	public void btnEditClick(OrdersStatus o) {
		selectedRow = o;
	}
	
	public void confirmDeleteClick() {
		try {
			session.deleteOrderStatus(selectedRow);
			init();
		} catch(Exception ex) {
	...
	public void btnDeleteClick(OrdersStatus o) {
		selectedRow = o;
	}

	public List<OrdersStatus> getSlave() {
		return slave;
	}

	public void setSlave(List<OrdersStatus> slave) {
		this.slave = slave;
	}

	public OrdersStatus getSelectedRow() {
		return selectedRow;
	}

	public void setSelectedRow(OrdersStatus selectedRow) {
